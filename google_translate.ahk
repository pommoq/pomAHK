...
iconN()
Menu, TRAY, Tip, [Alt+T]  translate from clipboard
;สวัสดีชาวโลก Hello, World

; MsgBox, % GoogleTranslate("今日の天気はとても良いです")

; MsgBox, % GoogleTranslate(Clipboard)
; MsgBox, % GoogleTranslate(Clipboard, "auto", "en")

+!t::
  send, ^c
  Sleep, 100
  txt := Clipboard "`r`n`r`n"
  MessageGoogle := GoogleTranslate(txt, "th", "en")
  MessageGoogle := txt MessageGoogle "`r`n`r`n`r`n`r`n"
  Notify("google translate",MessageGoogle,30,"style=pom")
return
...
!t::
  ...
  Sleep, 100
  txt := Clipboard "`r`n`r`n"
  MessageGoogle := GoogleTranslate(txt, "en", "th")
  MessageGoogle := txt MessageGoogle "`r`n`r`n`r`n`r`n"
  Notify("google translate",MessageGoogle,30,"style=pom")
  ; Notify("ToolTip",MessageGoogle,0,"style=ToolTip")
  ; Notify("BalloonTip",MessageGoogle,0,"style=BalloonTip")
  ; Notify("Error",MessageGoogle,0,"style=Error")
  ; Notify("Warning",MessageGoogle,0,"style=Warning")
  ; Notify("Info",MessageGoogle,0,"style=Info")
  ; Notify("Question",MessageGoogle,0,"style=Question")
  ; Notify("Progress",MessageGoogle,0,"style=Progress")
  ; Notify("Huge",MessageGoogle,0,"style=Huge")
  ; Notify("Load",MessageGoogle,0,"style=Load")
return
...
Notify(Title="Notify()",Message="",Duration="",Options="")
{
 ...
 If (Options)			; skip parsing steps if Options param isn't used
 {
  ...
  IH= IN= XC= XS= XW= PC= PB= " Options "Style=")
   ...
   Else If Style = Huge
    Return % Notify(Title,Message,Duration,"SI=100 ST=200 SC=200 GC=FFFFAA BC=00000 GR=27 BR=39 BW=6 BT=105 TS=18 MS=18" Options "Style=")
   Else If Style = Pom
    Return % Notify(Title,Message,Duration,"SI=250 GC=Default BC=00000 BK=00000 GR=0 BR=0 BW=1 BT=255 TS=16 MS=16 AX=1 XC=666666 " Options "Style=")
   Else If Style = Load
 ...
 ; ทำให้ขั้นแค่ตัวเดียว
 Gui, %GF%:Destroy  ; ปิดตัวเริ่ม
 GN := GF	          ; ให้สร้างจากเลขที่เริ่มต้น
 GNList .= "|" GN
 GN2 := GN + GL - GF + 1
